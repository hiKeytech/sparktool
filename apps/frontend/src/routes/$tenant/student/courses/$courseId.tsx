...
export function CourseDetails() {
  ...
  const startPaystackTransaction = (
    config: Omit<PaystackTransactionConfig, "onClose" | "onSuccess">,
    onSuccess: () => void,
  ) => {
  ...
  const handleEnrollClick = () => {
    ...
    if (!isFree && tenant.config?.monetization?.model === "pay-per-course") {
      ...
      startPaystackTransaction(
        getPaystackConfig(course?.price || 0, "enrollment"),
        () => {

...
function StudentLiveSessionsPage() {
  ...
  const getDisplayStatus = (session: LiveSession): LiveSessionStatus => {
  ...
  const upcomingSessions = sessions.filter(
    (session) => getDisplayStatus(session) === "scheduled",
  );

  const activeSessions = sessions.filter(
    (session) => getDisplayStatus(session) === "active",
  );

  const pastSessions = sessions.filter(
    (session) => getDisplayStatus(session) === "ended",
  );
    ...
    >
      <Stack gap="sm">
        <Group align="flex-start" justify="space-between">
          ...
          <Badge
            color={statusColors[getDisplayStatus(session)]}
            variant="light"
          >
            {getDisplayStatus(session).charAt(0).toUpperCase() +
              getDisplayStatus(session).slice(1)}
          </Badge>
        </Group>

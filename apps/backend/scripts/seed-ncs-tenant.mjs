...
async function loadPlatformAdminSeed() {
  const raw = await readFile(platformAdminSeedUrl, "utf8");
  return JSON.parse(raw);
}
...
async function upsertSeedUser(usersCollection, passwordAuthCollection, user) {
  const userId = user.id ?? buildSeedUserId(user);
  const now = Date.now();
...
function buildSeedUserId(user) {
  if (user.role === "super-admin") {
    return `platform-${user.email.split("@")[0]}`;
  }

  const [tenantId = "platform"] = user.tenantIds ?? [];
  return `${tenantId}-${user.role}-${user.email.split("@")[0]}`;
}
...
async function seedTenant() {
  ...
  const platformAdmin = await loadPlatformAdminSeed();
  ...
  try {
    ...
    const platformAdminId = await upsertSeedUser(
      usersCollection,
      passwordAuthCollection,
      platformAdmin,
    );
    ...
    for (const user of users) {
      await upsertSeedUser(usersCollection, passwordAuthCollection, user);
    }
    ...
    console.log(`Platform admin route: /login`);
    console.log(
      `Platform admin login: ${platformAdmin.email} / ${platformAdmin.password}`,
    );

...
async function main() {
  ...
  try {
    ...
    console.log(
      `   Current score: ${attempt.score}, passed: ${attempt.passed}\n`,
    );

    // 2. Load the quiz
    console.log("2. Loading quiz...");
    const quizzesCollection = db.collection("quizzes");
    const quiz = await quizzesCollection.findOne({ id: QUIZ_ID });
    ...
    questions.forEach((q, i) => {
      ...
      console.log(
        `       Correct answer: ${q.correctAnswer} (type: ${typeof q.correctAnswer})`,
      );
